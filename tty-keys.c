...
void tty_keys_build(struct tty *tty) {
  ...
  if (tty->key_tree != NULL) tty_keys_free(tty);
  tty->key_tree = NULL;
...
static void tty_keys_free1(struct tty_key *tk) {
  if (tk->next != NULL) tty_keys_free1(tk->next);
  if (tk->left != NULL) tty_keys_free1(tk->left);
  if (tk->right != NULL) tty_keys_free1(tk->right);
  free(tk);
}
...
static struct tty_key *tty_keys_find1(struct tty_key *tk, const char *buf,
  ...
  if (tk->ch == *buf) {
    ...
    if (len == 0 || (tk->next == NULL && tk->key != KEYC_UNKNOWN)) return (tk);
...
static int tty_keys_next1(struct tty *tty, const char *buf, size_t len,
  ...
  if (tk != NULL && tk->key != KEYC_UNKNOWN) {
    tk1 = tk;
    do log_debug("%s: keys in list: %#llx", c->name, tk1->key);
    while ((tk1 = tk1->next) != NULL);
    if (tk->next != NULL && !expired) return (1);
    *key = tk->key;
    return (0);
  }
  ...
  if (more == UTF8_MORE) {
    *size = ud.size;
    ...
    for (i = 1; i < ud.size; i++) more = utf8_append(&ud, (u_char)buf[i]);
    if (more != UTF8_DONE) return (-1);

    if (utf8_from_data(&ud, &uc) != UTF8_DONE) return (-1);
    *key = uc;
...
partial_key:
  ...
  /* Start the timer. */
  if (event_initialized(&tty->key_timer)) evtimer_del(&tty->key_timer);
...
static int tty_keys_extended_key(struct tty *tty, const char *buf, size_t len,
  ...
  *size = 0;
  ...
  for (end = 2; end < len && end != sizeof tmp; end++) {
    if (buf[end] == '~') break;
    if (!isdigit((u_char)buf[end]) && buf[end] != ';') break;
  }
  if (end == len) return (1);
  if (end == sizeof tmp || (buf[end] != '~' && buf[end] != 'u')) return (-1);
  ...
  if (buf[end] == '~') {
    if (sscanf(tmp, "27;%u;%u", &modifiers, &number) != 2) return (-1);
  } else {
    if (sscanf(tmp, "%u;%u", &number, &modifiers) != 2) return (-1);
  }
  *size = end + 1;
  ...
  if (nkey & KEYC_CTRL) {
    ...
    if (onlykey < 32 && onlykey != 9 && onlykey != 13 && onlykey != 27)
      /* nothing */;
  ...
  *size = 0;
  ...
  if (buf[2] == 'M') {
    ...
    *size = 3;
     ...
     * tmux.
     */
    if (sgr_type == 'm' && MOUSE_WHEEL(sgr_b)) return (-2);
  } else
    return (-1);
...
static int tty_keys_clipboard(struct tty *tty, const char *buf, size_t len,
  ...
  *size = end + terminator;
  ...
  if (~tty->flags & TTY_OSC52QUERY) return (0);
  ...
  for (i = 0; i < c->clipboard_npanes; i++) {
    ...
    if (wp != NULL) input_reply_clipboard(wp->event, out, outlen, "\033\\");
  }
...
static int tty_keys_device_attributes(struct tty *tty, const char *buf,
  ...
  *size = 0;
  if (tty->flags & TTY_HAVEDA) return (-1);
  ...
  switch (p[0]) {
    ...
    case 65: /* level 5 */
      for (i = 1; i < n; i++) {
        ...
        if (p[i] == 4) tty_add_features(features, "sixel", ",");
      }
      break;
  }
...
static int tty_keys_device_attributes2(struct tty *tty, const char *buf,
  ...
  *size = 0;
  if (tty->flags & TTY_HAVEDA2) return (-1);
...
static int tty_keys_extended_device_attributes(struct tty *tty, const char *buf,
  ...
  *size = 0;
  if (tty->flags & TTY_HAVEXDA) return (-1);
  ...
  for (i = 0; i < (sizeof tmp) - 1; i++) {
    if (4 + i == len) return (1);
    if (buf[4 + i - 1] == '\033' && buf[4 + i] == '\\') break;
    tmp[i] = buf[4 + i];
  }
  if (i == (sizeof tmp) - 1) return (-1);
  tmp[i - 1] = '\0';
  *size = 5 + i;
 ...
 * failure, 1 for partial.
 */
static int tty_keys_colours(struct tty *tty, const char *buf, size_t len,
                            size_t *size) {
  struct client *c = tty->client;
  u_int i;
  char tmp[128];
  ...
  *size = 0;
  ...
  for (i = 0; i < (sizeof tmp) - 1; i++) {
    if (5 + i == len) return (1);
    if (buf[5 + i - 1] == '\033' && buf[5 + i] == '\\') break;
    if (buf[5 + i] == '\007') break;
    tmp[i] = buf[5 + i];
  }
  if (i == (sizeof tmp) - 1) return (-1);
  if (tmp[i - 1] == '\033')
